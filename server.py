    ...
    if protocol == 'miot':
        for p in properties:
            if not set_miot_property(ip, p['siid'], p['piid'], p['value']):
                error(f'set_miot_property on {ip} failed')
                return
    elif protocol == 'midea':
        for p in properties:
            ...
            if not set_midea_property(ip, p['id'], type(getattr(midea_acs[ip], p['id']))(p['value'])):
                return
        apply_midea(ip)
